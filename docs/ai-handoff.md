...
SSH 到 Windows 由 `SSH_PROXY_MODE` 控制。学校/同一内网用 `direct`，离校或直连失败才用 `socks` 走 EZ4Connect。Mac 到 OpenAI 的代理仍通过 `wsl-mac-proxy-tunnel.sh` 反向给远端 WSL 使用。
...
- 不要去承诺 SSH 在 Mac 切热点瞬间保持同一条 TCP 不断。无论是 direct SSH 还是 SSH over EZ4Connect SOCKS，网络接口切换都可能拆旧 socket；正确目标是 tmux/会话可恢复。
...
- 不要让默认 logout 影响用户 Mac 端登录。`win-codex logout` 和远端 wrapper 的 `codex logout` 默认只移走远端 WSL 的 `auth.json`，并杀掉旧的 `codex_*` tmux 会话；真正上游退出登录必须显式 `--revoke`。
- 用户在 Mac 本地 `codex logout/login` 仍可能让已经打开的远端 Codex TUI 报 refresh token revoked。这是服务端 token 被撤销加旧进程不热更新；用 `win-codex reauth`，它会清理 auth、杀旧 tmux、重新网页登录。
- 如果 OAuth 浏览器能打开但远端报 `Token exchange failed`，或者登录后没有生成 `~/.codex/auth.json`，用 `win-codex hard-reauth`。它会把整个远端 `~/.codex` 移到 `~/.codex-bridge-backups/codex-reset-<时间>`，再用干净目录重新登录。
- 不要用 device-code 登录，当前 Team 禁用了。
- 不要把 EZ4Connect 密码、私钥、Codex auth 文件提交进仓库。
- 如果 GUI 启动的 `zju-connect` 带 `-disable-keep-alive`，用 `ez4-vpn restart` 切到受控启动器。这个命令沿用本机 EZ4Connect 配置文件，但仓库里不保存真实密码。受控启动器现在是 terminal-loop + supervisor 双层守护：`zju-connect` 崩溃由 supervisor 拉起，supervisor 异常退出由 terminal-loop 拉起。
